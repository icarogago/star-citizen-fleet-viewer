...
const ShipCard: React.FC<ShipCardProps> = ({ ship, ownerCount, owners }) => {
  return (
    <Card className="group bg-black/20 backdrop-blur-sm border-blue-500/20 md:hover:border-blue-400/40 transition-none md:transition-all md:duration-300 md:hover:scale-105 md:hover:shadow-xl overflow-hidden cursor-pointer">
      ...
      <CardContent className="p-4 space-y-3">
        <div>
          <h3 className="text-xl font-bold text-white dark:text-white text-gray-900 md:group-hover:text-blue-300 md:transition-colors">
            {ship.name}
          </h3>
            ...
            {owners.map((owner, index) => (
              <div key={index} className="flex items-center gap-2">
                <img
                  ...
                  className="w-4 h-4"
                />
                <span className="dark:text-blue-300 text-blue-900">[NF] {owner}</span>
              </div>
            ))}
          </div>
        </div>
